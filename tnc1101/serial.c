...
void set_serial_parameters(serial_t *serial_parameters, char *serial_device, speed_t serial_speed)
// ------------------------------------------------------------------------------------------------
{
    serial_parameters->SERIAL_TNC = open(serial_device, O_RDWR | O_NOCTTY | O_NONBLOCK);

    if (serial_parameters->SERIAL_TNC < 0)
    {
    ...
    // Set Baud Rate 
    cfsetospeed (&serial_parameters->tty, serial_speed);
    cfsetispeed (&serial_parameters->tty, serial_speed);
...
void close_serial(serial_t *serial_parameters)
// ------------------------------------------------------------------------------------------------
{
    close(serial_parameters->SERIAL_TNC);
}